...
- the C-13 contract addendum, which limits historical C-13 closure to the intent-clarifier seam and does not widen the committed file surface
...
| `C-1/C-2/C-3/C-4/C-9/C-12` | Differentiated analyst, research, and lens prompts stay inside the shared runtime envelope while adding the approved methodology and injection-safety contracts | `src/keystone/deliberation/analyst.py`; `src/keystone/research/research_agent.py`; `src/keystone/specification/prompts/decompose_*_lens.md` | `tests/unit/deliberation/test_analyst.py`; `tests/unit/research/test_research_agent.py`; `tests/e2e/test_mock_pipeline.py` | `closed in 65a612d` |
| `C-13` | Step 4 intent-clarifier recovery is cleared only at the current intent-clarifier seam: `evidence_would_change` is preserved in the prompt and `IntentClarificationResult`, but downstream structural propagation into later specification/task-generation logic is not claimed by this clearance | `src/keystone/specification/prompts/intent_clarification.md`; `src/keystone/specification/intent_clarifier.py` | `tests/unit/specification/test_intent_clarifier.py::TestIntentClarifier::test_result_includes_evidence_would_change` | `cleared at the intent-clarifier seam only in 65a612d` |
...
- [candidate-65a612d-c13-contract-addendum.md](candidate-65a612d-c13-contract-addendum.md): `AUTHORITY NOTE`
...
- Any downstream C-13 structural propagation beyond the intent-clarifier seam remains outside the historical `65a612d` clearance surface.
